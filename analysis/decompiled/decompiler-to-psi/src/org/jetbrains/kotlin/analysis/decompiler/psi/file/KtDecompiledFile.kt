...
private fun <T : PsiElement> copyStubRecursively(
    ...
    val clonedStub = originalStub.copy(newParentStub)
    checkWithAttachment(
        originalStub::class == clonedStub::class,
        { "${originalStub::class.simpleName} is expected, but ${clonedStub::class.simpleName} is found" },
    )

    for (originalChild in originalStub.childrenStubs) {
        copyStubRecursively(originalStub = originalChild, newParentStub = clonedStub)
    }
