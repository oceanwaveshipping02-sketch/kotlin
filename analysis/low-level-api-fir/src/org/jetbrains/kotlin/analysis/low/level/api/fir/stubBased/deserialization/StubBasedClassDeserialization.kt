...
/**
 * Gets or calculates compiled stub for [this] element and casts it to [S].
 *
...
private fun <S, T> T.calculateStub(): Stub where T : StubBasedPsiElementBase<in S>, T : KtElement, S : StubElement<*> {
    ...
    requireWithAttachment(containingFile.isCompiled, { "Expected compiled file" }) {
        withPsiEntry("ktFile", containingFile)
    }

    // `let` is used to hold the stub tree reference on the stack
    return containingFile.calcStubTree().let {
        val stub = greenStub
