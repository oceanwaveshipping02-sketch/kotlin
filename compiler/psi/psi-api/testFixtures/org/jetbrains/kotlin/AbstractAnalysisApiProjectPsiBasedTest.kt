...
import org.jetbrains.kotlin.AbstractAnalysisApiProjectPsiBasedTest.SourceDirectory
 ...
 *
 * See [AbstractAnalysisApiProjectPsiBasedTestWithMasterFile] and [AbstractAnalysisApiProjectPsiBasedTestWithCodeBaseValidation]
 */
abstract class AbstractAnalysisApiProjectPsiBasedTest<T : SourceDirectory> : KtUsefulTestCase() {
    ...
    sealed class SourceDirectory(val sourcePaths: List<String>) {
        class ForCodeBaseValidation(sourcePaths: List<String>) : SourceDirectory(sourcePaths)
        class ForCheckAgainstMasterFile(sourcePaths: List<String>, val outputFilePath: String) : SourceDirectory(sourcePaths)
    }
 ...
 * Then compares this text against the contents of the master file [SourceDirectory.ForCheckAgainstMasterFile.outputFilePath].
 *
 * If comparison fails, throws an exception with [getErrorMessage] as the message.
 */
abstract class AbstractAnalysisApiProjectPsiBasedTestWithMasterFile :
    AbstractAnalysisApiProjectPsiBasedTest<SourceDirectory.ForCheckAgainstMasterFile>() {
    override fun SourceDirectory.ForCheckAgainstMasterFile.traverse(psiManager: PsiManager, fileSystem: VirtualFileSystem) {
        val roots = getRoots()
    ...
    private fun SourceDirectory.ForCheckAgainstMasterFile.getExpectedFile(): File {
        val homeDir = KtTestUtil.getHomeDirectory()
        return File(homeDir, outputFilePath)
    }

    abstract fun SourceDirectory.ForCheckAgainstMasterFile.getErrorMessage(): String
 ...
 */
abstract class AbstractAnalysisApiProjectPsiBasedTestWithCodeBaseValidation :
    AbstractAnalysisApiProjectPsiBasedTest<SourceDirectory.ForCodeBaseValidation>() {
    override fun SourceDirectory.ForCodeBaseValidation.traverse(psiManager: PsiManager, fileSystem: VirtualFileSystem) {
        val roots = getRoots()
